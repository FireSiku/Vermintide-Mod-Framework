...
function HookObject:disable()
    self._registry.active[self._orig] = false
end
function HookObject:is_active()
    return self._registry.active[self._orig]
end

local function create_hook_object(self, orig, hook_type)
    local obj = setmetatable({}, {__index = HookObject})
    obj._mod = self
    obj._orig = orig
    obj._registry = _registry[self][hook_type]
...
local function create_hook(self, orig, obj, method, handler, hook_type)
    ...
    return create_hook_object(self, orig, hook_type)
end
...
function HookMixin:new_before(obj, method, handler)
    ...
    local hook_object = create_hook(self, orig, obj, method, handler, HOOK_TYPE_BEFORE)
...
function HookMixin:new_after(obj, method, handler)
    ...
    local orig = get_orig_function(self, obj, method)
    local hook_object = create_hook(self, orig, obj, method, handler, HOOK_TYPE_AFTER)
...
function HookMixin:new_hook(obj, method, handler)
    ...
    local orig = get_orig_function(self, obj, method)
    local hook_object = create_hook(self, orig, obj, method, handler, HOOK_TYPE_NORMAL)
...
function HookMixin:new_rawhook(obj, method, handler)
    ...
    local orig = get_orig_function(self, obj, method)
    local hook_object = create_hook(self, orig, obj, method, handler, HOOK_TYPE_RAW)
